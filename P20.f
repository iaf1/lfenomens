C     IVAN ALSINA FERRER
      ...
      PARAMETER(L=60)
      INTEGER*2 S(1:L,1:L)
      REAL*8 GENRAND_REAL2, MAGN,MAGNE,ENERGIA
      ...
      OPEN(10,FILE="P1-configuration.conf")
      DO I=1,L
            DO J=1,L
                  IF (S(I,J).EQ.1) THEN
                        WRITE(10,101) I,J
 ...
 101  FORMAT(I5,1X,I5)
      ...
      DO I=1,L-1
            DO J=1,L-1
                  ENERGIA = ENERGIA + S(I,J)*S(I+1,J)
                  ENERGIA = ENERGIA + S(I,J)*S(I,J+1)
            ENDDO
      ENDDO

      DO I=1,L-1
            ENERGIA = ENERGIA + S(L,I)*S(1,I)
            ENERGIA = ENERGIA + S(L,I)*S(L,I+1)
            ENERGIA = ENERGIA + S(I,L)*S(I,1)
            ENERGIA = ENERGIA + S(I,L)*S(I+1,L)
      ENDDO

      ENERGIA = ENERGIA + S(L,L)*S(1,L)
      ENERGIA = ENERGIA + S(L,L)*S(L,1)
